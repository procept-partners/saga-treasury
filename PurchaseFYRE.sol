...
contract PurchaseFYRE is Ownable {
     ...
     */
    function setExchangeRates(uint256 newUsdcRate, uint256 newEthRate, uint256 newTbtcRate) external onlyOwner {
        require(newUsdcRate > 0 && newEthRate > 0 && newTbtcRate > 0, "Rates must be positive");
    ...
    function purchaseFYREWithTBTC(uint256 tbtcAmount) external {
        ...
        require(tbtcToken.balanceOf(msg.sender) >= tbtcAmount, "Insufficient tBTC balance");
