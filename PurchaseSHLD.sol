...
contract PurchaseSHLDProxy is Ownable {
    ...
    // Event to log SHLD purchases for cross-chain processing, now without a signature
    event SHLDPurchaseRecorded(
        ...
        bytes32 dataHash                // Hash of the purchase data for off-chain signing
    );
     ...
     * Transfers FYRE tokens from the sender to the treasury and logs the purchase details.
    ...
    function purchaseSHLD(uint256 fyreAmount) external {
        ...
        // Generate the data hash to be signed off-chain
        bytes32 dataHash = keccak256(
            abi.encodePacked(purchaseCounter, msg.sender, fyreAmount, shldAmount, block.timestamp)
        );

        // Emit an event with the data hash (no on-chain signature)
